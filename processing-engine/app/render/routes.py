"""FITS render/read routes (thumbnail, preview, histogram, pixeldata, cubeinfo).

Moved verbatim from main.py (CE plan Phase 2): as module-level @app routes
they registered regardless of CE_MODE and needed per-handler guards. As a
router they mount only in the full-engine (non-CE) configuration —
deny-by-default is now purely structural, with the CE middleware as backstop.
The CE /api shims resolve dataIds and call these handler functions directly.
"""
