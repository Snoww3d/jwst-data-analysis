...
- `POST /api/mast/search/{target|coordinates|observation|program|facets}` · `POST /api/mast/whats-new` · `GET /api/mast/coverage[?bbox=]` (Phase 5 sky map: whole-sky HEALPix density grid, or real footprints inside `bbox`; 202 + `Retry-After` while the daily snapshot builds) (target search resolves featured-name aliases; `filters` is the closed `MastCriteria` whitelist; `facets` is the position-less search — no date facet/`daysBack` → last 90 days, flagged `default_window_applied`)
...
- **MAST Search**: `/mast/search/target`, `/coordinates`, `/observation`, `/program`, `/facets` (criteria-only, bounded to 90 days unless dated) | `POST /mast/whats-new` - new observations | `GET /mast/coverage[?bbox=]` - sky-coverage snapshot (grid / footprints) | `POST /mast/products` - data products for observations
...
**Quick Start**: Open the public `/search` page (nav "Search", the Discover home CTA, or the library toolbar's "Search MAST" link; `/archive` redirects) -> Select search type (target/coordinates/observation/program) -> Search -> Import selected observations (login required to import).
